...
public class SmithWatermanGotoh {
	...
	public static Alignment align(Sequence s1, Sequence s2, Matrix matrix,
		...
		// Инициализируем границы матрицы трассировки для остановки.
		// столбец по i от 0 до m-1 заполняем элемениты с периодом n (!)  - 0, n, 2*n, 3*n, .... , (m-1)*n 
		...
		// массив для вертикальных вставок - размером матрицы m*n
		short[] sizesOfVerticalGaps = new short[m * n];
		// массив для горизонтальных вставок - размером матрицы m*n
		short[] sizesOfHorizontalGaps = new short[m * n];
		
		// цикл заполнения массива размером m*n
		// заполняем последовательно значением 1 по m элементов (в строке) повторяя n раз (по количеству столбцов)   
		for (int i = 0, k = 0; i < m; i++, k += n) {
			for (int j = 0; j < n; j++) {
		...
		// Строим матрицу переходов, переходы записываются в массив pointers  
		Cell cell = sw.construct(s1, s2, scores, o, e, pointers,
				sizesOfVerticalGaps, sizesOfHorizontalGaps);
		
		// проходим матрицу в обратном порядке, чтобы получить выравненные последовательности
		Alignment alignment = sw.traceback(s1, s2, matrix, pointers, cell,
				sizesOfVerticalGaps, sizesOfHorizontalGaps);
		
		// заполняем свойства матрицы
	...
	private Cell construct(Sequence s1, Sequence s2, float[][] matrix, float o,
		...
		float f; // score of alignment x1...xi to y1...yi if xi aligns to yi -- вес выравнивания xi на yi
		float[] g = new float[n]; // score if xi aligns to a gap after yi -- массив n (по строке) весов пропуска в последовательности 1 - xi после сивола yi
		float h; // score if yi aligns to a gap after xi -- вес для пропуска в последовательности 2
		float[] v = new float[n]; // best score of alignment x1...xi to y1...yi -- массив лучшего выравнивания x на y
		float vDiagonal;
		...
		// цикл построения матрицы по строке 1 (m шагов)
		for (int i = 1, k = n; i < m; i++, k += n) {
			...
			for (int j = 1, l = k + 1; j < n; j++, l++) {
				...
				// используем простую матрицу MATCH - совпадение 1, не совпадение -1  
	...
	private Alignment traceback(Sequence s1, Sequence s2, Matrix m,
		...
		// цикл по матрице от последней ячейки
		while (stillGoing) {
